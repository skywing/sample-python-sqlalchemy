...
@pytest.fixture(scope='class')
def test_actor_table(engine: Engine):
    with engine.connect() as conn:
        metadata = MetaData()
...
class TestTable:
    def test_actor_insert(self, engine: Engine, test_actor_table: Table):
        with engine.connect() as conn:
            result = conn.execute(
    ...
    def test_actor_select(self, engine: Engine, test_actor_table: Table):
        with engine.connect() as conn:
